...
-- Run this after existing setup_tables.sql
...
-- Reviews table (unchanged but included for completeness)
...
BEGIN
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING(booking_reference, 9) AS UNSIGNED)), 0) + 1
    INTO counter
    FROM bookings 
    WHERE booking_reference LIKE CONCAT(today_date, '%');
    
    SET ref_num = CONCAT(today_date, LPAD(counter, 4, '0'));
...
BEGIN
    IF NEW.booking_reference IS NULL OR NEW.booking_reference = '' THEN
        SET NEW.booking_reference = generate_booking_reference();
    END IF;
    
    -- Set auto-cancel time if not set
    IF NEW.auto_cancel_after IS NULL THEN
        SET NEW.auto_cancel_after = DATE_ADD(NOW(), INTERVAL 48 HOUR);
    END IF;
    
    -- Calculate deposit and remaining amounts based on payment mode
    IF NEW.payment_mode = 'deposit_50' THEN
        SET NEW.deposit_amount = NEW.total_amount * 0.5;
        SET NEW.remaining_amount = NEW.total_amount * 0.5;
    ELSE
        SET NEW.deposit_amount = NEW.total_amount;
        SET NEW.remaining_amount = 0.00;
    END IF;
END//
DELIMITER ;
...
SELECT 'Enhanced booking system database schema created successfully!' as Status;
SELECT 'Tables: bookings, payments, addon_services, promo_codes, time_slots, business_settings, notifications, reviews, activity_logs' as Created;
SELECT 'Ready for professional booking flow implementation!' as NextStep;